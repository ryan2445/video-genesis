    ...
    <div class="relative">
        ...
        <v-hover v-slot="{ hover }" :open-delay="100">
            <div v-if="bannerSrc" class="w-full h-32 relative">
                ...
                <v-fade-transition>
                    <v-btn v-if="isRootUser && (hover || bannerPictureLoading)" icon
                        absolute right outlined small elevation="4"
                        style="bottom:6px; backdrop-filter: blur(8px);"
                        @click="onBannerEdit" :loading="bannerPictureLoading"
                        :disabled="bannerPictureLoading">
                        <v-icon small>icon-pencil-outline</v-icon>
                    </v-btn>
                </v-fade-transition>
                <v-file-input class="absolute" ref="bannerEditFile" dense flat
                    hide-details hide-input hide-spin-buttons :loader-height="0"
                    prepend-icon accept="image/png, image/jpeg, image/bmp"
                    :rules="fileRules" @change="onBannerChange" />
            </div>
        </v-hover>
        <v-hover v-slot="{ hover }" :open-delay="100">
            <div v-if="userProfilePic" class="-bottom-16 left-14 absolute w-44 h-44 z-10">
                <img class="w-full h-full object-cover rounded-full" :src="userProfilePic"
                    alt="avatar" style="border: solid 2px #e1e1e1;" />
                <v-fade-transition>
                    <v-btn v-if="isRootUser && (hover || profilePictureLoading)" icon
                        absolute outlined small elevation="4"
                        style="bottom:12px; right:22px; backdrop-filter: blur(8px);"
                        @click="onProfilePictureEdit" :disabled="profilePictureLoading"
                        :loading="profilePictureLoading">
                        <v-icon small>icon-pencil-outline</v-icon>
                    </v-btn>
                </v-fade-transition>

                <v-file-input class="absolute" ref="profileEditFile" dense flat
                    hide-details hide-input hide-spin-buttons :loader-height="0"
                    prepend-icon accept="image/png, image/jpeg, image/bmp"
                    :rules="fileRules" @change="onProfilePictureChange" />
            </div>
        </v-hover>
    </div>
</template>
