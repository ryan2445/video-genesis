<template>
    <v-card style="padding: 14px 12px" min-width="360px" class="relative">
        <v-progress-linear absolute top indeterminate rounded :active="loading"
            color="orange lighten-1" />
        <v-card-title class="justify-center">
            <h3 v-text="`Sign ${type === 'signUp' ? 'up' : 'in'} for Video Genesis`" />
        </v-card-title>
        <v-card-text class="text-center">
            <validation-observer ref="validationObservation" v-slot="{ invalid }">
                <v-form ref="signupForm" @submit.prevent="onSubmit">
                    <!-- Username Input -->
                    <validation-provider name="Username" rules="required|min:3"
                        v-slot="{ errors }">
                        <v-text-field id="usernameAuth" :error-messages="errors"
                            placeholder="Username" :success="!errors" v-model="username"
                            color="orange lighten-1">
                        </v-text-field>
                    </validation-provider>

                    <!-- Email Input -->
                    <validation-provider v-if="type === 'signUp'" name="Email"
                        rules="required|email" v-slot="{ errors }">
                        <v-text-field id="emailAuth" :error-messages="errors"
                            placeholder="Email" :success="!errors" v-model="email"
                            @keydown.enter="onSubmit" color="orange lighten-1">
                        </v-text-field>
                    </validation-provider>
                    ...
                    <div v-if="type == 'signIn'">
                        <v-btn x-small text color="blue lighten-1"
                            @click="resetPassword()">
                    ...
                    <!-- Submit Button -->
                    <v-btn @click="onSubmit" :disabled="invalid" class="mt-3 white--text"
                        color="orange lighten-1">
                        Submit
                    </v-btn>
                </v-form>
