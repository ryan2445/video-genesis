    ...
    <v-card-text>
      <validation-observer ref="validationObservation" v-slot="{ invalid }">
        <v-form ref="uploadForm" @submit.prevent="handleSubmit">
          ...
          <v-checkbox label="make private" v-model="isPrivate" color="#A13440">
          </v-checkbox>
          ...
          <div class="flex justify-center gap-x-2">
            <v-btn @click="handleCancel" class="mt-3 text-gray-900" text>
              Cancel
            </v-btn>
            <v-btn
              @click="handleSubmit"
...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      this.$store.commit("videos/videosSet", [
        { ...video, user: this.$store.getters["users/rootUser"] },
        ...this.$store.getters["videos/videos"],
      ]);
