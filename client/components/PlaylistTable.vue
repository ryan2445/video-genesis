<template>
  <div
    className="playlist-table-container w-full h-screen flex flex-1 flex-grow bg-white"
      ...
      <v-list flat width="100%">
        ...
        >
          ...
          <v-list-item-content two-line>
            <v-list-item-title>
              <NuxtLink
                :to="`videos/pk=${playlistItem.video.pk}&sk=${playlistItem.video.sk}`"
              >
...
export default {
  ...
  methods: {
    onThumbnailClick(video) {
      this.$router.push(`/videos/pk=${video.pk}&sk=${video.sk}`);
    },
    async onDelete(video) {
      ...
      try {
        ...
        const response = await this.$store.dispatch(
          "playlists/playlistDeleteVideos",
          params
        );
      } catch (e) {
